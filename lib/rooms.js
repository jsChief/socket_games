...
module.exports = function createRooms(deps) {
  ...
  function toRoom(room, event, payload) {
    ...
    toSpectators(room, event, payload);
  }

  // Emit a socket event to every spectator watching a room.
  function toSpectators(room, event, payload) {
    for (const sp of room.spectators) {
      if (sp && sp.id) io.to(sp.id).emit(event, payload);
    }
  }
  ...
  // Serializable snapshot of a room for the room-view / spectator view.
