...
module.exports = function createPizza(deps) {
  ...
  // Public (non-spoiling) state for spectators: each player's probe grid shows
  // whether attacked cells were hits or misses, but NOT hidden slice placement.
  function buildSpectate(room) {
    const pz = room.pizza;
    const attacked = [0, 1].map((i) =>
      pz.attacked[i].map((a, c) => (a ? pz.boards[i][c] : null)),
    );
    return {
      active: pz.active,
      phase: pz.phase,
      turn: pz.turn,
      submitted: pz.submitted.slice(),
      found: pz.found.slice(),
      attacked,
      names: room.players.map((p) => (p ? p.name : null)),
    };
  }
  function emitSpectate(room) {
    toSpectators(room, "spectate-pizza", buildSpectate(room));
  }
  function spectateTo(socket, room) {
    if (room.pizza && room.pizza.active) {
      socket.emit("spectate-pizza", buildSpectate(room));
    }
  }
